...
function nestedSchema (paths, field, cleanTree, value, prefix) {
  ...
  if (value.type && value.type === Map) {
    cleanTree[field] = getCleanTree(paths[prefix + field].options.of.tree, paths[prefix + field].options.of.paths, '')
    cleanTree['type'] = 'object'
  } else if (paths[prefix + field] && paths[prefix + field].schema && paths[prefix + field].schema.tree && paths[prefix + field].schema.paths) {
...
Generator.prototype.generateMapping = function generateMapping (schema) {
  ...
  if (schema.statics.dynamic_templates) {
    return { dynamic_templates: schema.statics.dynamic_templates, properties: mapping }
  }
